...
Definition qpoly_of
  (R : fieldType) (phi : {poly R}) := {poly_((size phi).-1) R}.
Notation "{ 'qpoly' phi }" :=
  (qpoly_of phi) (at level 0, format "{ 'qpoly'  phi }").

Section qpoly_ring.
Context {R : fieldType}.
Variable phi : {poly R}.
Hypothesis H : size phi > 1.
Lemma phi_neq0 : phi != 0.
rewrite -size_poly_eq0.
apply/negP => /eqP Hc.
by move: Hc H => ->.
Qed.
Hint Resolve phi_neq0 : core.
Implicit Types (p q : {qpoly phi}).
(* Polynomial ring structure. (over phi) *)

Local Lemma modphi_qpolyR (s : {poly R}) :
  s %% phi \is a poly_of_size (size phi).-1.
Proof.
...
Hypothesis H : size phi > 1.
Hint Resolve phi_neq0 : core.
Canonical qpoly_ringType_phi := qpoly_ringType H.
Canonical qpoly_comRingType_phi := qpoly_comRingType H.

Local Definition rm_qp :
  rmorphism (qpolify H : [ringType of {poly R}] -> [ringType of {qpoly phi}]).
repeat constructor; move=> *; apply/eqP.
* by rewrite eqE /qpolify /= ?(modp_mod, modp_add)
            -opprB addrC !opprD opprK modp_opp.
* by rewrite eqE /qpolify /= ?(modp_mod, modp_mul)
           [X in _ == X %% _]mulrC modp_mul mulrC.
Defined.
